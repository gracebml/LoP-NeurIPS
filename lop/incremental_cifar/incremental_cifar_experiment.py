...
class IncrementalCIFARExperiment(Experiment):
    ...
    def _initialize_summaries(self):
        ...
        # Dashboard metrics (computed at each task boundary via compute_task_metrics)
        self.results_dict["metrics_before"] = []
        self.results_dict["metrics_after"] = []
        self._prev_ref_outputs = None
        self._ntk_ref_batch = None
    ...
    def train(self, train_dataloader: DataLoader, test_dataloader: DataLoader, val_dataloader: DataLoader,
        ...
        # Compute BEFORE metrics at the very start
        self._compute_before_metrics(training_data)
    ...
    def _compute_before_metrics(self, training_data):
        """Compute BEFORE-task metrics using the dashboard."""
        self.net.eval()
        if self._ntk_ref_batch is None:
            for sample in DataLoader(training_data, batch_size=64, shuffle=True):
                self._ntk_ref_batch = sample["image"].to(self.device)
                break
        ref_batch = self._ntk_ref_batch
        features = []
        self.net.forward(ref_batch, feature_list=features)
        self._task_before_metrics = compute_task_metrics(
    ...
    def extend_classes(self, training_data: CifarDataSet, test_data: CifarDataSet, val_data: CifarDataSet):
        ...
        if (self.current_epoch % self.class_increase_frequency) == 0:
            ...
            # Update prev_ref_outputs for next task's churn
            with torch.no_grad():
                self._prev_ref_outputs = self.net.forward(ref_batch).detach()

            # Store metrics
            self.results_dict["metrics_before"].append(
                getattr(self, '_task_before_metrics', after_metrics))
            self.results_dict["metrics_after"].append(after_metrics)

            # Get loss/acc from most recent epoch
            e = self.current_epoch - 1
            task_loss = self.results_dict["test_loss_per_epoch"][e].item()
            train_acc = self.results_dict["train_accuracy_per_checkpoint"][
                max(0, self.current_running_avg_step - 1)].item()
            test_acc = self.results_dict["test_accuracy_per_epoch"][e].item()

            print_task_summary(
                task_boundary_idx,
                self.results_dict["metrics_before"][-1],
                after_metrics,
                task_loss, train_acc, test_acc)
            self.net.train()

            # Compute BEFORE metrics for the NEXT task period
            self._compute_before_metrics(training_data)
