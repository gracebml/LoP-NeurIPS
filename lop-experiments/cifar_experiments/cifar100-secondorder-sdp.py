...
CONFIGS = {
    ...
    'asam_sdp': dict(
        **_SHARED,
        optimizer='asam',
        lr=0.05, rho=0.1, momentum=0.9, weight_decay=5e-4,
        sdp_gamma=SDP_GAMMA, lr_schedule='cbp_style',
    ),
    # ─── 8. ASAM (no SDP) ───
    'asam_nosdp': dict(
    ...
    for epoch in range(start_epoch, config['num_epochs']):
        ...
        for sample in train_loader:
            ...
            if opt_type == 'sassha':
                ...
                def closure():
                    optimizer.zero_grad()
                    p2 = net(img)[:, all_classes[:current_num_classes]]
                    l2 = loss_fn(p2, tgt)
                    l2.backward()
                    return l2
...
METHOD_STYLES = {
    ...
    'asam_sdp':         {'color': '#4CAF50', 'ls': '-',  'lw': 2.2, 'label': 'ASAM+SDP'},
    'asam_nosdp':       {'color': '#4CAF50', 'ls': '--', 'lw': 1.6, 'label': 'ASAM'},
